  ...
  return (
    <div className="p-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <StatCard icon="🏛️" label="Firme contabilitate" value={stats.firmeActive} sub={`din ${stats.firmeTotal} total`} color="text-indigo-700" />
        <StatCard icon="👥" label="Clienți activi" value={stats.clientiActivi} sub={`din ${stats.clientiTotal} total`} color="text-green-700" />
        <StatCard icon="⏸️" label="Firme suspendate ONRC" value={stats.firmeSuspendate} color="text-yellow-700" />
        <StatCard icon="💰" label="Venit estimat lunar" value={`${stats.venitEstimat.toFixed(0)} RON`} sub="din clienți activi" color="text-blue-700" />
      </div>

      {/* Status clienti */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <StatCard icon="✅" label="Activi" value={stats.clientiActivi} color="text-green-600" bgColor="bg-green-50" />
        <StatCard icon="⏸️" label="Suspendați" value={stats.clientiSuspendati} color="text-yellow-600" bgColor="bg-yellow-50" />
        <StatCard icon="❌" label="Reziliați" value={stats.clientiReziliati} color="text-red-600" bgColor="bg-red-50" />
        <StatCard icon="🎯" label="Prospecți" value={stats.clientiProspecti} color="text-blue-600" bgColor="bg-blue-50" />
      </div>
